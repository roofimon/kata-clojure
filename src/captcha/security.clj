...
(defn string-integer? [pattern] (= pattern 2))

(defn string-operand [index] (get m (- index 1)) )

(def integer-operand (fn [index] (str index)))

(defn assign-left-operand [pattern left]
  (if (string-integer? pattern)
    (string-operand left)
    (integer-operand left) ))

(defn assign-right-operand [pattern right]
  (if (string-integer? pattern)
    (integer-operand right)
    (string-operand right) ))

(defn assign-operator [operator]
  (get ["+" "*" "-"] (- operator 1)))

(def captcha (fn [pattern left operator right]
